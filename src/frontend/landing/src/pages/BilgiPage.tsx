...
import { fetchBilgiLandingData, submitBilgiFeedback, type BilgiLandingData } from '../api'
...
function FeedbackForm({ lang }: { lang: Lang }) {
  const t = lang === 'tr'
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setStatus('idle')
    try {
      await submitBilgiFeedback(type, message.trim())
      setStatus('ok')
      ...
      <div style={styles.card}>
        <form onSubmit={handleSubmit}>
          ...
          <textarea
            ...
            placeholder={t ? 'Dilek, istek veya şikayetinizi yazın...' : 'Write your feedback...'}
          />
          <button type="submit" disabled={sending} style={{ ...styles.linkBtn, border: 0, cursor: sending ? 'wait' : 'pointer' }}>
      ...
      <div style={styles.pageContent}>
        ...
        {data.googleReviewsUrl && (
          <section style={styles.section}>
            ...
            >
              {t ? 'Google\'da değerlendir' : 'Rate on Google'}
            </a>
          </section>
        )}
